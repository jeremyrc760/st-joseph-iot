...
describe('Azure IoT Integration', () => {
  ...
  const deviceConnectionString = process.env.IOT_DEVICE_CONNECTION_STRING;

  const eventHubConnectionString =
    process.env.IOT_HUB_EVENTHUB_CONNECTION_STRING;

  const eventHubName = process.env.IOT_HUB_EVENTHUB_NAME;

  const consumerGroup = process.env.IOT_HUB_TEST_CONSUMER_GROUP;
  ...
  it('should publish telemetry to IoT Hub and consume the same message', async () => {
    ...
    if (
      ...
      throw new Error('Azure IoT integration configuration is missing');
    }
    ...
    const receivedMessagePromise = new Promise<unknown>((resolve, reject) => {
    ...
    const deviceClient = Client.fromConnectionString(
      deviceConnectionString,
      MqttWs,
    );
    ...
    try {
      ...
      const message = new Message(JSON.stringify(testTelemetry));
      ...
      await new Promise<void>((resolve, reject) => {
        deviceClient.sendEvent(message, (error) => {
          if (error) {
            reject(error);
            return;
          }
      ...
      const receivedTelemetry = await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () =>
              reject(new Error('Timed out waiting for Azure IoT telemetry')),
            15000,
          ),
        ),
      ]);
